...
状态：已完成；本设计文档保留为 M6 的历史设计依据
日期：2026-05-03

> **新阶段归属：** 本文档属于 `S2（核心实证工作台）`。`M6` 保留为当前活跃里程碑编号，不单独承担长期阶段叙事。

> 历史收口约束：M6 实施期间，不开启 M7（面板成熟化）或更高里程碑。统一 `fit` 接口需向后兼容——现有 `fit_ols_file` 在 M6 内标记为 deprecated wrapper，不立即删除。公式预处理不得破坏现有 `@formula` 宏的直接使用方式。该约束现已完成收口。
