...
/** 加载数据集并更新 stores 的公共逻辑 */
async function loadDatasetIntoStores(
  filePath: string,
  feedback: FeedbackFn,
): Promise<boolean> {
  setLoading(true);
  try {
    ...
    useMessageStore.getState().addMessage({
      kind: 'result',
      command: `use ${filePath}`,
      result: { glance: null as any, tidy: [], warnings: [], diagnostics: {} },
    });
    try {
      ...
      useMessageStore.getState().addMessage({
        kind: 'data',
        command: '',
        data_result: describeResult as import('../types/protocol').DataResult,
      });
    } catch {
      feedback('warning', '自动描述失败，可手动执行 describe 命令');
    }
  } catch (e: unknown) {
...
export const handleUse: CmdHandler = (parsed, _input, feedback) => {
  ...
  if (!filePath) {
    return (async () => {
      ...
      return loadDatasetIntoStores(filePath, feedback);
    })();
  }
...
export const handleExport: CmdHandler = (parsed, input, feedback) => {
  ...
  if (format === 'plot') {
    ...
    return (async () => {
      ...
      feedback('success', `图表已触发下载：${fname}（已保存到浏览器默认下载目录）`);
      return true;
    })();
  }
  ...
  return (async () => {
    ...
    try {
      ...
      feedback('success', '导出内容已下载到浏览器默认下载目录，并在结果流中显示摘要。');
      setError(null);
    } catch (e: unknown) {
  ...
  return (async () => {
    ...
    try {
      const result = await api.fitModel({
        ...
        weightsColumn: spec.weights || String(p.weights_column ?? ''),
        clusterColumn: spec.cluster_column || '',
        panelId: String(p.panel_id ?? ''),
        panelTime: String(p.panel_time ?? ''),
        panelMethod: String(p.panel_method ?? ''),
        instruments: Array.isArray(p.instruments) ? p.instruments.join(',') : String(p.instruments ?? ''),
        endogColumns: Array.isArray(p.endog_columns) ? p.endog_columns.join(',') : String(p.endog_columns ?? ''),
        gmmWeight: String(p.gmm_weight ?? ''),
        instrumentLags: p.instrument_lags as [number, number] | undefined,
        dpgmmStyle: String(p.dpgmm_style ?? ''),
        collapseInstruments: p.collapse_instruments as boolean | undefined,
        treatmentColumn: String(p.treatment_column ?? p.treated_column ?? ''),
        postColumn: String(p.post_column ?? ''),
        eventTimeColumn: String(p.event_time_column ?? ''),
        outcomeColumn: String(p.outcome_column ?? ''),
        propensityFormula: String(p.propensity_formula ?? ''),
        outcomeFormula: String(p.outcome_formula ?? ''),
        timeColumn: String(p.time_column ?? ''),
        tsVariable: String(p.variable ?? ''),
        tsVariables: Array.isArray(p.variables) ? p.variables.join(',') : '',
        tsLags: p.lags as number | undefined,
        tsDeterministic: String(p.deterministic ?? 'constant'),
        tsMethod: String(p.ts_method ?? ''),
        orderP: (p.order as number[] | undefined)?.[0],
        orderD: (p.order as number[] | undefined)?.[1],
        orderQ: (p.order as number[] | undefined)?.[2],
        strataColumn: String(p.strata_column ?? ''),
        psuColumn: String(p.psu_column ?? ''),
        fpcColumn: String(p.fpc_column ?? ''),
        equations: p.equations as string[] | undefined,
        systemEndogenous: p.system_endogenous as string[][] | undefined,
        systemInstruments: p.system_instruments as string[][] | undefined,
        surMaxIter: p.sur_max_iter as number | undefined,
        surTol: p.sur_tol as number | undefined,
        quantileTau: p.quantile_tau as number | undefined,
        nlsFamily: String(p.nls_family ?? ''),
        nlsStart: p.nls_start as number[] | undefined,
        nlsMaxIter: p.nls_max_iter as number | undefined,
        nlsTol: p.nls_tol as number | undefined,
        thresholdVariable: String(p.threshold_variable ?? ''),
        thresholdGrid: p.threshold_grid as number[] | undefined,
        thresholdTrimFrac: p.threshold_trim_frac as number | undefined,
        archOrder: p.arch_order as number | undefined,
        garchP: p.garch_p as number | undefined,
        garchQ: p.garch_q as number | undefined,
        garchMaxIter: p.garch_max_iter as number | undefined,
        garchTol: p.garch_tol as number | undefined,
        spatialWeightsPath: String(p.spatial_weights_path ?? ''),
        spatialIdColumn: String(p.spatial_id_column ?? ''),
        spatialRowStandardize: p.spatial_row_standardize as boolean | undefined,
        durationTimeColumn: String(p.duration_time_column ?? ''),
        durationEventColumn: String(p.duration_event_column ?? ''),
      });
