...
function result_to_payload(result::NLSFitResult; include_augment::Bool = true)
...
function result_to_payload(result::ThresholdFitResult; include_augment::Bool = true)
    glance_table = result.glance_table
    tidy_table = result.tidy_table
    diag = copy(result.diagnostics)
    payload = _result_payload_core(glance_table, tidy_table, diag)
