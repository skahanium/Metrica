...
Metrica 的三层隔离（Core → Runtime → App）与结构化协议（`glance` / `tidy` / `augment`、结构化警告与错误）在代码中**持续落地**。`oss-maturity` 上已完成一轮「完善阶段」关键项：嵌套 `ModelSpec.params`、P0 门禁与 CI 对齐、薄弱包测试扩充、Runtime 校验分族、OLS golden 外部参考脚本等。
...
4. **CI 分层。**
   - `ci.yml`：18 个 Julia 包矩阵 `Pkg.test()`、`rust-check`（含 **`vertical_slice --test-threads=1`**）、`app-test`、`model-type-alignment`、`lint`（clippy + `npm run build`）。
   - `full-quality.yml`：定时全量 Julia 包测试（`workflow_dispatch` 可手动触发）。
   - 本地：`scripts/dev/test-p0.sh`、`make test-p0` 与 CI 主路径对齐。
...
- CI：**18/18** Julia 包 + Rust + App + alignment + lint。
...
| App ESLint | **未配置**；lint 用 `npm run build` 作类型检查（CI 注释 + CONTRIBUTING 已写明） |
| `full-quality` 与 `ci` | 夜间全量 vs PR 主路径；PR 不跑 `full-quality` 全部 job（设计如此，需知悉） |
...
|--------|------|----------------------|
| **P0** | 18 包 `Pkg.test()` + `vertical_slice` 串行进 CI | **已完成**（`ci.yml` + `test-p0.sh`） |
...
| **P3** | Phase 3/4/6 字面：`1/cond` 注释、CI build≠ESLint、serialize 补例 | **已完成**（`b808241`）；README 原生壳说明此前已有 |
...
| 6 | CI lint + README 壳说明 | 完成（lint = clippy + build，非 ESLint） |
...
| CI 无 lint | **不成立**（有 lint job；无 ESLint） |
...
| 测试覆盖 | **B** | CI 全包 + 薄弱包已补强；Data / 全族 golden 不足 |
...
| `1ae2c07` | P0：`test-p0.sh`、CI 18 包、vertical_slice 串行、CONTRIBUTING |
